...
def triplet_dict(a_ids, b_ids, counts):
    return {(int(ai), int(bi)): int(c) for ai, bi, c in zip(a_ids, b_ids, counts, strict=True)}
...
def check_equivalent(a, b):
    ...
    stardist_result = {(int(row), int(col)): int(dense[row, col]) for row, col in zip(rows, cols, strict=True)}
...
def sequential_masks(a, b):
    a_seq, _, _ = relabel_sequential(a)
    b_seq, _, _ = relabel_sequential(b)
    return np.ascontiguousarray(a_seq, dtype=np.uint32), np.ascontiguousarray(b_seq, dtype=np.uint32)
...
def bench_case(shape, n_ids, *, repeats=5):
    ...
    results = [
        ...
        ("StarDist relabel+overlap", timed(stardist_prepare, original_a, original_b, repeats=repeats)),
    ]
    ...
    print(f"  StarDist dense matrix      {dense_bytes / 1e6:9.1f} MB")


def sparse_id_stress(*, repeats=3):
    a, b = make_segmentation_like_masks((4096, 4096), n_ids=1_000, max_id=2_000_000_000)
    ...
    print(f"  NumPy unique               {numpy_time * 1e3:9.2f} ms  ({numpy_time / fast_time:.2f}x vs fastlabelops)")
